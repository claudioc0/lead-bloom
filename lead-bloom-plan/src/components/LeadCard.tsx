import { Bookmark, MessageSquarePlus, Youtube, Check, Loader2 } from "lucide-react";
...
export function LeadCard({ lead, onGenerate }: { lead: Lead; onGenerate: (l: Lead) => void }) {
  const { saveLead } = useLeadMutations();
  const isSaved = lead.isSaved ?? false;
    ...
    <div className="card-lift flex h-full flex-col gap-4 rounded-xl border border-border bg-card p-4">
      ...
      <div className="mt-auto flex gap-2">
        ...
        <button
          onClick={() => {
            if (isSaved) return;
            saveLead.mutate(lead.id, {
              onSuccess: () => toast.success(`${lead.name} saved to My Leads`),
