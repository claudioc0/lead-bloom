...
import { useDashboardStats, useMonthlyUsage } from "@/hooks/useDashboardStats";
import { trendPct } from "@/lib/lead-mappers";
...
function Index() {
  const { data: leads = [], isLoading } = useLeadsQuery({ limit: 20 });
  const { data: dash } = useDashboardStats();
  const { data: usage } = useMonthlyUsage();
  const [active, setActive] = useState<Lead | null>(null);
  const [sortAsc, setSortAsc] = useState(false);

  const leadsMonth = dash?.leads_month ?? 0;
  const messagesMonth = dash?.messages_month ?? 0;
  const contacted = dash?.contacted ?? 0;
  const responseRate = dash?.response_rate ?? 0;
  const planLimit = usage?.leads_limit ?? 50;
  ...
  const stats = [
    {
      ...
      icon: Target,
      accent: "text-primary",
      trend: trendPct(leadsMonth, dash?.leads_prev ?? 0),
      progress: { used: leadsUsed, limit: planLimit },
    },
    {
      label: "Messages Generated",
      value: messagesMonth.toString(),
      icon: MessageSquare,
      accent: "text-[color:var(--teal)]",
      trend: trendPct(messagesMonth, dash?.messages_prev ?? 0),
    },
    {
